---
name: issues
description: Optional Lazarus companion — turn a CODEBASE_AUDIT.md "Top 10 Action Items" (§11) into GitHub Issues. Reads the audit produced by the Lazarus `audit` skill, proposes one issue per ranked action item (carrying the item's validation command as its acceptance check), lets you ratify the set before anything is created, and dedups against existing issues so re-running after a re-audit never makes duplicates. Use when the user wants to turn an audit into a backlog, file the audit's action items as issues/tickets, or "create GitHub issues from the audit." GitHub Issues only — uses the gh CLI. REQUIRES a CODEBASE_AUDIT.md with a §11 section.
---
...
It turns the `## 11. Top 10 Action Items` section of a `CODEBASE_AUDIT.md` (produced by the `audit` skill) into GitHub Issues — one per item, each carrying the item's `validation command` as its acceptance check.
...
Read `CODEBASE_AUDIT.md` and extract each Top 10 Action Item: rank/number, priority, action, impact, effort (S/M/L), risk, files involved, and validation command. Keep the **rank** — it is the dedup key. Never invent a field the audit didn't provide; if an item has no validation command, carry it through as "no acceptance check specified."
...
Every issue this skill creates carries a hidden provenance marker in its body: `<!-- lazarus:audit-item:<rank> -->`. Build the set of ranks that already have an issue. **v1 policy is skip-if-exists:** an item whose rank already has an issue is NOT recreated — report it as "already filed" with a link to the existing issue. Do not edit, reopen, or close existing issues; a human may have changed them.

### 3. Propose the set and ratify (the gate)

Present a table of what *would* be created — rank, title (the action), priority/severity, effort, target labels — and which items are skipped as already-filed. Ask the user to:

- choose which items to file (default: all not-yet-filed),
...
- **body:** impact · risk · effort · files involved · the **validation command** under an explicit **"Acceptance check"** heading · a provenance footer: `From CODEBASE_AUDIT.md §11 item #<rank>, generated <date>.` followed by the hidden marker `<!-- lazarus:audit-item:<rank> -->`
...
- Inventing fields §11 didn't provide — carry "no acceptance check specified" rather than fabricating a validation command.
